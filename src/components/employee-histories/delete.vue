<template>
  <icon
    :icon="['fas', 'trash']"
    class="mx-2 cursor-pointer hover:text-red-600"
...
export default {
  ...
  methods: {
    ...mapActions(["deleteValue", "getValues"]),
    handleDelete() {
      const payload = {
        idEmployee: this.$route.params.id,
        status: "completed",
        page: this.valueLinks.currentPage
      };

      if (this.creator === this.userId) {
        this.deleteValue(this.id)
          .then(() =>
            this.getValues(payload).then(res => {
              if (res.data.data.length === 0) {
                const preventPage = {
                  userId: this.userId,
                  page: payload.page - 1
                };
                this.getValues(preventPage);
              }
            })
          )
          .catch(err => {
